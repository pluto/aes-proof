...
fn encrypt_tls(message: &[u8], key: &[u8], iv: &[u8], seq: u64) -> Vec<u8> {
    ...
    let ciphertext = cipher
        .encrypt(nonce, aes_payload).expect("error generating ct");
...
pub fn aes_witnesses(
   ...
   let message = "test000000000000test000000000000";
   let seq = 1;
   let ct = encrypt_tls(message.as_bytes(), key_ascii.as_bytes(), iv_ascii.as_bytes(), seq);
   ...
   use ctr;
   use aes::cipher::{
       KeyIvInit, StreamCipher, BlockEncrypt, KeyInit,
   ...
   use aes_gcm_siv::{aead::{Payload as SIVPayload, Aead}, Aes256GcmSiv};
   ...
   let nonce = GenericArray::from_slice(&iv_bytes_short_256);
   let aes_payload = Payload {msg: &message_bytes_256, aad: &[]};
   let ciphertext = cipher.encrypt(nonce, aes_payload).expect("error generating ct");
   ...
   println!("AES GCM IV={:?}, tm={:?}, ct={:?}", hex::encode(ghash_iv), hex::encode(tag_mask),  hex::encode(buffer));
   ...
   println!("AES GCM 256 SIV: ct={:?}, bytes={:?}", hex::encode(ciphertext_siv.clone()), ciphertext_siv);
